...
@interface RSECreditCardViewController ()
...
@implementation RSECreditCardViewController
...
- (void)showIDViewController {
    //    RTJYIdentificationCustomCameraController *vc = [[RTJYIdentificationCustomCameraController alloc]
    //                                                    initWithType:CustomCameraTypeBack];
    //    vc.delegate = self;
...
//    cv::Mat srcImage;
//    UIImageToMat(image,srcImage,false);
//
////    // 图片裁剪
////    cv::Mat szImage = srcImage(cv::Rect(46, 292, 774, 78));
//
//    //图像灰度化
//    cv::Mat grayImage;
...
//    // 滤波
//    cv::Mat blurImage;
//    GaussianBlur(grayImage, blurImage, cv::Size(3, 3), 0, 0);
//
//
//
//    // 形态学梯度
...
//    morphologyEx(blurImage, gradImage, cv::MORPH_GRADIENT, morphKernel);
//
////    UIImage *image1 = MatToUIImage(gradImage);
////    self.imageView.image = image1;
//
//    // 二值化
//    cv::Mat binImage;
//    threshold(gradImage, binImage, 0.0, 255.0, cv::THRESH_BINARY | cv::THRESH_OTSU);
//
//
//    cv::Mat destImage;
//    cv::bitwise_not(binImage,destImage);
//
//
//    UIImage *newImage = MatToUIImage(destImage);
//    self.imageView.image = newImage;
//
//
//    // Create RecognitionOperation
//    CGRect cardNumberRect = CGRectMake(46, 292, 774, 78);
//
//    CGRect dateRect = CGRectMake(314, 440, 210, 50);
//
//
...
//    operation.tesseract.sourceResolution;
//    operation.tesseract.charWhitelist = @"1234567890";
//    operation.tesseract.rect = cardNumberRect;
...
//            self.nameLabel.text =[@"银行卡号:" stringByAppendingString: [recognizedTesseract recognizedText]];
//        }
//    };
//
//
...
//
//    // Configure inner G8Tesseract object as described before
//    operation2.tesseract.charWhitelist = @"1234567890/";
//    operation2.tesseract.image = newImage;
//    operation2.tesseract.delegate=self;
//    operation2.tesseract.rect = dateRect;
//    // Setup the recognitionCompleteBlock to receive the Tesseract object
//    // after text recognition. It will hold the recognized text.
...
//        if ([recognizedTesseract recognizedText].length > 0) {
//            self.IDNumberLabel.text = [@"到期时间:" stringByAppendingString:[recognizedTesseract recognizedText]];
//        }
//    };
//
//
...
- (void)userDidProvideCreditCardInfo:(CardIOCreditCardInfo *)info inPaymentViewController:(CardIOPaymentViewController *)paymentViewController {
    ...
    self.nameLabel.text = info.cardNumber;
    
    self.IDNumberLabel.text = [NSString stringWithFormat:@"%ld/%ld",info.expiryMonth,info.expiryYear];
    //[NSString stringWithFormat:@"Received card info. Number: %@, expiry: %02lu/%lu, cvv: %@.", info.redactedCardNumber, (unsigned long)info.expiryMonth, (unsigned long)info.expiryYear, info.cvv];
